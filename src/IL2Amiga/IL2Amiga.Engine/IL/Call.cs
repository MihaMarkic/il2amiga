...
namespace IL2Amiga.Engine.IL
{
    ...
    {
        public static uint GetStackSizeToReserve(MethodBase method, Type? type = null)
        {
            var methodInfo = method as MethodInfo;
            uint returnSize = 0;
            if (methodInfo is not null)
            {
                returnSize = SizeOfType(methodInfo.ReturnType);
            }
            if (returnSize == 0)
            {
                return 0;
            ...
            int xExtraStackSize = (int)Align(returnSize, 4);
            var xParameters = method.GetParameters();
            ...
            if (!methodInfo!.IsStatic)
            {
                ...
                {
                    if (IsReferenceType(method.DeclaringType!))
                    {
                        xExtraStackSize -= GetObjectReferenceSize();
                    }
                    else
        ...
        //    // todo: implement exception support
        //    uint xExtraStackSize = GetStackSizeToReserve(aTargetMethod);
