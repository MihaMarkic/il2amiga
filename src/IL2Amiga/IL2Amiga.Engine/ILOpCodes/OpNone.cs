...
namespace IL2Amiga.Engine.ILOpCodes
{
    public class OpNone : ILOpCode
    {
        ...
        public OpNone(Code aOpCode, int aPos, int aNextPos, ExceptionRegionInfoEx? aCurrentExceptionRegion)
        ...
        public override void DoInterpretStackTypes()
        {
            switch (OpCode)
            {
                ...
                case Code.Ldelem_Ref:
                    ...
                    {
                        throw new Exception("Ldelem Array type is not an array (Actual = " + typeArray.FullName + ")");
                    }
                    StackPushTypes[0] = typeArray.GetElementType()!;
                    break;
                case Code.Not:
                case Code.Neg:
                    if (StackPushTypes[0] != null)
